        ...
        <div className="overflow-hidden">
          ...
          >
            {data.archetypes.map((archetype, i) => (
              ...
              >
                <ImageryArchetypeCard archetype={archetype} />
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-6">
              ...
              {data.archetypes.map((archetype, i) => (
                <button
                  ...
                  className={`h-0.75 flex-1 rounded-full transition-colors cursor-pointer ${
                    i === index ? "bg-secondary-green" : "bg-black/10 hover:bg-black/20"
                  }`}
                />
              ))}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="w-8 h-8 rounded-full flex items-center justify-center text-neutral-grey-20 hover:text-primary-black transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              >
                <ArrowLeft size={20} />
              </button>
              <button
